  ...
  options={[
    {
      ...
      correct: false,
      explanation: "Não. Autenticação responde 'quem é você' e autorização responde 'o que você pode fazer'.",
    },
    {
      text: "Autenticação confirma uma identidade, autorização verifica o que essa identidade tem permissão de fazer",
      correct: true,
    },
    {
      text: "Autorização vem antes da autenticação",
      correct: false,
      explanation: "A ordem natural é autenticar primeiro (saber quem é) e depois autorizar (saber o que pode).",
    },
  ]}
/>

<Checkpoint
  challenge="Escolha uma rota da sua aplicação que só deveria ser acessível para administradores. Se um usuário comum fizer a chamada direta na API (ignorando a interface), a sua aplicação nega o acesso? Como?"
  slug="ciberseguranca-para-desenvolvedores"
  nodeId="autenticacao-autorizacao"
/>
